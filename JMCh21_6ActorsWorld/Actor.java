...
public class Actor implements Comparable<Actor>
{
    ...
    public void readMail()
    {
        ...
        while (mailbox.isEmpty() != true)
        {
            Message msg = mailbox.remove();
            String str = mailbox.remove().getText();
            String str1 = mailbox.remove().getText().substring(5);
            if (str.contains( "have" ))
            {
                if (countPossesions(item ) == 0)
                {
            ...
            {
                if (countPossessions > 1)
                {
                    send(msg.getSender() + "have")
                }
            }
        }
